---
layout: default
title: Certificaciones Oficiales
permalink: /certificados/
---

# 🎓 Certificaciones y Títulos
**Formación Académica y Cursos Técnicos.**

Aquí puedes consultar la documentación oficial de mis titulaciones y cursos de especialización.

<div style="display: flex; flex-wrap: wrap; gap: 20px; justify-content: flex-start;">

  <div style="background: rgba(255, 255, 255, 0.05); border: 1px solid #30363d; border-radius: 8px; padding: 20px; width: 300px; flex-grow: 1;">
    <h3 style="margin-top: 0;">🏛️ Grado Ingeniería Informática</h3>
    <p style="color: #8b949e; font-size: 0.9em;">Universidad de Murcia</p>
    <p>Especialización en Ingeniería del Software y Tecnologías de la Información.</p>
    <a href="/assets/certificados/grado-informatica.pdf" target="_blank" style="background: #238636; color: white; padding: 8px 16px; text-decoration: none; border-radius: 6px; display: inline-block; margin-top: 10px; font-weight: bold;">📄 Ver Título Oficial</a>
  </div>

  <div style="background: rgba(255, 255, 255, 0.05); border: 1px solid #30363d; border-radius: 8px; padding: 20px; width: 300px; flex-grow: 1;">
    <h3 style="margin-top: 0;">📈 Marketing Digital</h3>
    <p style="color: #8b949e; font-size: 0.9em;">Benowu International</p>
    <p>Curso intensivo sobre SEO, SEM y estrategias de crecimiento digital.</p>
    <a href="/assets/certificados/marketing-digital.pdf" target="_blank" style="background: #1f6feb; color: white; padding: 8px 16px; text-decoration: none; border-radius: 6px; display: inline-block; margin-top: 10px; font-weight: bold;">📄 Ver Certificado</a>
  </div>

  <div style="background: rgba(255, 255, 255, 0.05); border: 1px solid #30363d; border-radius: 8px; padding: 20px; width: 300px; flex-grow: 1;">
    <h3 style="margin-top: 0;">🇬🇧 Inglés B2 (First)</h3>
    <p style="color: #8b949e; font-size: 0.9em;">Cambridge English</p>
    <p>Certificación de nivel intermedio-alto con competencia profesional.</p>
    <a href="/assets/certificados/ingles-b2.pdf" target="_blank" style="background: #1f6feb; color: white; padding: 8px 16px; text-decoration: none; border-radius: 6px; display: inline-block; margin-top: 10px; font-weight: bold;">📄 Ver Certificado</a>
  </div>

</div>

<br>
<hr>

### 📜 Otros Cursos y Seminarios
* **Introducción a la IA Generativa** - Google Cloud Skills [Ver PDF](#)
* **Seguridad en Redes** - Cisco Academy [Ver PDF](#)

[🔙 Volver al Inicio](/)
